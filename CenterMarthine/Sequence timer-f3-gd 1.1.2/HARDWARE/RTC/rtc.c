...
u8 RTC_Init(void)
{
    ...
    if (BKP_ReadBackupRegister(BKP_DR1) != 0x5000)		//从指定的后备寄存器中读出数据:读出了与写入的指定数据不相乎
        {
            BKP_DeInit();	//复位备份区域
            RCC_LSEConfig(RCC_LSE_ON);	//设置外部低速晶振(LSE),使用外设低速晶振
            while (RCC_GetFlagStatus(RCC_FLAG_LSERDY) == RESET&&temp<250)	//检查指定的RCC标志位设置与否,等待高速晶振就绪
                {
                    temp++;
                    delay_ms(10);
                }
            if(temp>=250)return 1;//初始化时钟失败,晶振有问题
            RCC_RTCCLKConfig(RCC_RTCCLKSource_LSE);		//设置RTC时钟(RTCCLK),选择HSE/128作为RTC时钟  8M/128=62.5K=62500
            ...
            RTC_SetPrescaler(32767); //设置RTC预分频的值(RTC_PRLH与RTC_PRLL,共20位)，最后1Hz,在这调精度
            RTC_WaitForLastTask();	//等待最近一次对RTC寄存器的写操作完成
            RTC_Set(2017,9,29,12,0,0);  //设置时间
            RTC_ExitConfigMode(); //退出配置模式
            BKP_WriteBackupRegister(BKP_DR1, 0X5000);	//向指定的后备寄存器中写入用户程序数据
        }
    else//系统继续计时
        {
    ...
    if (RTC_GetITStatus(RTC_IT_SEC) != RESET)//秒钟中断
        {
            ...
            if(flag_dev.OPEN_PAGE_STR==1)
                {
                    if(flag_dev.screen_flag==HOME&&flag_dev.page_num==1)
                        {

                            LCD_ShowNum(255,15,calendar.hour,2,24,WHITE,RED);
                            LCD_ShowNum(290,15,calendar.min,2,24,WHITE,RED);

                        }
                    else if(flag_dev.screen_flag==HOME_X)
                        {
                            LCD_ShowNum(255,15,calendar.hour,2,24,WHITE,RED);
                            LCD_ShowNum(290,15,calendar.min,2,24,WHITE,RED);
                        }
...
//	if(RTC_GetITStatus(RTC_IT_ALR)!= RESET)//闹钟中断--------------需要使能闹钟中断；
//	{
...
void RTCAlarm_IRQHandler(void)
{
    ...
    if(RTC_GetITStatus(RTC_IT_ALR) != RESET)
        {
            ...
            if((Current_Seccount<Alarm_Sequence.Sec_MAX)&&(Current_Seccount>Alarm_Sequence.Sec_MID))
				...
				//					LCD_Show_3030_String(0,10,"3",BLACK);
                }
            else if((Current_Seccount<Alarm_Sequence.Sec_MID)&&(Current_Seccount>Alarm_Sequence.Sec_MIN))
                {
                    ...
                    flag_dev.RTC_Alarm_Dex=Alarm_Sequence.MID_Alarm_Dex;							 
						//			 LCD_Show_3030_String(0,10,"2",BLACK);
                }
            else if(Current_Seccount<Alarm_Sequence.Sec_MIN)
                {
				//					LCD_Show_3030_String(0,10,"1",BLACK);
    ...
    EXTI_ClearITPendingBit(EXTI_Line17);//line17是链接到RTC闹钟事件中断
    RTC_WaitForLastTask();
